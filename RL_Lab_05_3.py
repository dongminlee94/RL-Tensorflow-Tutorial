...
The same set of parameter values and initial conditions
                                    will lead to an ensemble of different outputs.
...
그러므로 멘토가 하는 말을 10%만 듣고 나머지는 내가 가고 싶은대로 방향으로 90%를 학습할 것입니다.
...
먼저 밑에 있는 stochastic(non-deterministic) Q-learning Algorithm 공식을
Q[state, action] = (1 - learning_rate) * Q[state, action]
    + learning_rate * (reward + dis * np.max(Q[new_state, :]))
이 공식보다는 이해하기가 더 쉬운 분배법칙을 적용한 공식으로 보겠습니다.

즉 코드로 구현해보면,
Q[state, action] = Q[state, action] + learning_rate
   * (reward + dis * np.max(Q[new_state, :]) - Q[state, action])

여기서 learning rate 뒤에 있는 것을 TD error라고 합니다.(참고)

따라서 밑에 분배법칙을 적용한 공식으로 보면 learning rate를 조절할 수 있습니다.
하지만 너무 많이 올리면 overshooting이 될 수 있습니다. 따라서 조절하여 설정해야합니다.
"""
dis = .99
num_episodes = 2000
...
for i in range(num_episodes):
    ...
    while not done:
        ...
        """
        이 부분이 바로 stochastic(non-deterministic) Q-learning Algorithm 공식을
        적용한 코드(분배법칙 전)입니다.
        """
        # Q[state, action] = (1 - learning_rate) * Q[state, action] \
             # + learning_rate * (reward + dis * np.max(Q[new_state, :]))

        """ 분배법칙을 적용한 공식을 코드로 표현하였습니다. """
        Q[state, action] = Q[state, action] + learning_rate \
           * (reward + dis * np.max(Q[new_state, :]) - Q[state, action])
