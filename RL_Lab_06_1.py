...
두 번째로, input으로 State만 주고 neural network(= linear regression,
            활성화함수를 안쓰고 선형함수로 적용하면 결국 같다.)를 돌려
            모든 Action값(Q-value)를 구하는 것. 여기서는 두 번째 방법을 사용할 것입니다.

결국 network로 한다는 것은 Supervised Learning으로써
output값으로 Q-learning value(non-deterministic Q-learning Algorithm이 아닌
                                처음에 배운 deterministic Q-learning)
input값으로는 State만 주어서 neural network를 거친 값이 'y = R + r maxQ(s`)'와 같아지도록
즉, neural networ(= linear regression)를 거친 cost를 converge하도록 만드는 Algorithm입니다.

여기서 'non-deterministic Q-learning Algorithm이 아닌
처음에 배운 deterministic Q-learning' 라고 했는데
이 이유는 cost를 converge되도록 만드는 것과 non-deterministic Q-learning Algorithm과
사실상 같은 작업을 하기 때문입니다.
...
"""
위의 코드는 위에서 언급했던 'output값으로 Q-learning value(non-deterministic
Q-learning Algorithm이 아닌 처음에 배운 deterministic Q-learning)'을 하기 위해
공간을 만들어 놓은 코드입니다.
밑에서 'Qs[0, a] = reward + dis * np.max(Qs1)' 부분과
'sess.run(train, feed_dict={X: one_hot(s), Y: Qs})' 부분에서 담아질 것입니다.
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while not done:
            ...
            """ 여기서 위에 있는 Qs[0, a]중에 a(action)만 학습을 시킵니다.
            그 이유는 내가 한 action에 대한 Q값만 update를 시키면 되기 때문입니다.
            """
