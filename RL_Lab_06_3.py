...
# input_size는 6_2에서 봤던 4가지가 input_size이기 때문에 '4'가 됩니다.
...
for episode in range(max_episodes):
    ...
    """
    print(state)
    여기서 프린트를 찍어본 이유는 밑에 'x = np.reshape(state, [1, input_size])'에서
    reshape을 하는 이유가 궁금해서 했습니다.
    [0.02016351 -0.03426143  0.00718547  0.02116652] 이런 식으로 state가 찍힙니다.
    2차원 배열로 바꿔주어야 합니다.
    우리는 [[0.02016351 -0.03426143  0.00718547  0.02116652]]식으로 바꿔주어야
    원핫인코딩처럼 되어 feed_dict에 넣을 수 있습니다.
    """
    done = False
    ...
    while not done:
        ...
        """
        Q[0, action] = -100 와 Q[0, action] = reward + dis * np.max(Q_next)가 label
        즉, Q-learning 값이 될 것입니다.
        """
        ...
        Neural Network(= linear regression, 여기서는 활성화함수를 안썼지만,
        선형함수를 쓰고 있으므로 결국 NN = linear regression이다. 여기서만!!)을 통해서
