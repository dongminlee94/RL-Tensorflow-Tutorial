...
# 알고리즘 중에서 DFS, BFS를 안다면 이해하기 더 수월합니다.
# [1], [2], [3], [4], [5], [6]을 잘 따라오세요~!

import gym
import numpy as np
import matplotlib.pyplot as plt
# 여기서 갑자기 에러가 떳지만 'sudo apt-get install python3 -tk'으로 해결하였습니다.
from gym.envs.registration import register
import random as pr

# random argmax에 대한 함수입니다.
# 만약에 Qrkqtdl rkxekaus random하게 설정하기 위해서 만든 것입니다.
...
for i in range(num_episodes):
    ...
    state = env.reset() # [2] 1 epoch마다 초기화를 시켜줍니다.
    rAll = 0
    done = False

    # The Q-Table learning algorithm
    while not done: # 게임이 끝나지 않을 때까지 계속 반복시킵니다.
        action = rargmax(Q[state, :]) # [3] action은 rargmax중에 가장 큰 값의 '방향' 으로 취합니다.

        # Get new state and reward from environment
        new_state, reward, done, _ = env.step(action) # [4] reward와 다음 state(s`)를 만듭니다.
